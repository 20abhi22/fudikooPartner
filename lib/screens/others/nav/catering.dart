...
import 'package:fudiko/components/catering_all_enquiry_box.dart';
...
import 'package:fudiko/components/catering_confirmed_box.dart';
import 'package:fudiko/components/catering_sent_box.dart';
...
class Catering extends StatefulWidget {
  ...
  const Catering({super.key, this.onDrawerTap, this.partnerProfile});
...
class _CateringState extends State<Catering> implements TabBackHandler {
  ...
  List<CateringEnquiryModel> _allEnquiries = [];
  ...
  List<CateringEnquiryModel> _searchResults = [];
  ...
  Future<void> _pickCustomDateRange() async {
    ...
    final picked = await showDialog<DateTimeRange>(
      context: context,
      builder: (context) => _CateringDateRangeDialog(
                  ...
                  return CateringSentBox(
                    ...
                    onAccept: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Error confirming party response"),
                          ),
                        );
                      }
                    },
                    onDelete: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Error deleting sent enquiry"),
                          ),
                        );
                      }
                    },
                    ...
                    child: _responsiveContent(
                      ...
                      child: Container(
                        ...
                        padding: EdgeInsets.symmetric(
                          horizontal: Breakpoints.isMobile(width) ? 40.w : 36,
                          vertical: Breakpoints.isMobile(width) ? 28.h : 26,
                        ),
                        decoration: BoxDecoration(
                          color: Colors.white,
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(
                              height: Breakpoints.isMobile(width) ? 20.h : 18,
                            ),
                            Row(
                              children: [
                                Expanded(
                                  child: SizedBox(
                                    height: Breakpoints.isMobile(width)
                                        ? 35.h
                                        : 36,
                                    child: AppButton(
                                      text: "Yes",
                                ...
                                SizedBox(
                                  width: Breakpoints.isMobile(width)
                                      ? 20.w
                                      : 18,
                                ),
                                Expanded(
  ...
  Widget _buildStatusSearchSwitcher() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final threeRowsHeight = (tabHeight * 3) + (rowGap * 2) + 8.0;
        final thirdRowTop = (tabHeight + rowGap) * 2;
        final searchHeight = isWideShortPhone ? 56.0 : 60.h;
...
class _CateringDateRangeDialog extends StatefulWidget {
  final DateTime? initialStartDate;
  final DateTime? initialEndDate;
  final DateTime firstDate;
  final DateTime lastDate;

  const _CateringDateRangeDialog({
  ...
  @override
  State<_CateringDateRangeDialog> createState() =>
      _CateringDateRangeDialogState();
}

class _CateringDateRangeDialogState extends State<_CateringDateRangeDialog> {
